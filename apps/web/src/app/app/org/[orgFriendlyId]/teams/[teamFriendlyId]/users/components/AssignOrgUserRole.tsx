...
import { OrgUserRoles } from '@services/organizations/organization.type';
import { useToast } from '@context/ToastContext';
import { useUpdateOrganizationUserRole } from '@queries/useOrganizationsQuery';
...
export default function AssignOrgUserRole({
  ...
  const handleRoleUpdate = async (userId: string, data: OrgUserRoles) => {
    try {
      const reqBody = {
  ...
  const onRoleChange = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedRole) {
      const data: OrgUserRoles = {
        role: selectedRole as 'org-admin' | 'org-member',
      };
