...
import {
  ...
  type FlowbiteDatepickerViewsDecadesTheme,
} from './Views/Decades';
import {
  DatepickerViewsMonth,
  type FlowbiteDatepickerViewsMonthsTheme,
} from './Views/Months';
import {
  DatepickerViewsYears,
  type FlowbiteDatepickerViewsYearsTheme,
} from './Views/Years';
...
> = (
  ...
  const theme = mergeDeep(datePickerTheme, customTheme);
        ...
        {!inline && (
          <TextInput
            ...
            onFocus={() => {
              ...
              if (propValue) {
                setViewDate(new Date(propValue as string));
                setSelectedDate(new Date(propValue as string));
              }
              setIsOpen(true);
            }}
            value={
              propValue
                ? getFormattedDate(
                    language,
                    new Date(propValue as string) as unknown as Date,
                  )
                : value && getFormattedDate(language, value)
            }
            readOnly
            onChange={customOnchange}
            {...props}
